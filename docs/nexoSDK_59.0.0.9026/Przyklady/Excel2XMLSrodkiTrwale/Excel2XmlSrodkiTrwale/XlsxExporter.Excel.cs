...
namespace Excel2XmlSrodkiTrwale
{
    public partial class XlsxExporter
    {
        ...
        /// <summary>
        /// Metoda pobierająca wiersze z danymi z arkusza kalkulacyjnego o numerze podanym jako drugi argument.
        /// </summary>
        /// <param name="spreadsheetDocument">Arkusz kalkulacyjny.</param>
        /// <returns>Zwraca wiersze z pierwszego arkusza podanego arkusza kalkulacyjnego.</returns>
        private static IEnumerable<Row> GetSpreadsheetRows(SpreadsheetDocument spreadsheetDocument, int number)
        {
            WorkbookPart workbookPart = spreadsheetDocument.WorkbookPart;

            IEnumerable<Sheet> sheetcollection = spreadsheetDocument.WorkbookPart.Workbook.GetFirstChild<Sheets>().Elements<Sheet>();

            string relationshipId = sheetcollection.First(d => d.SheetId.Value == number).Id.Value;
            WorksheetPart worksheetPart = (WorksheetPart)spreadsheetDocument.WorkbookPart.GetPartById(relationshipId);
        ...
        /// <summary>
        /// Metoda zwracajaca ilośc zakładek w arkuszu kalkulacyjnym
        /// </summary>
        /// <param name="spreadsheetDocument">Arkusz kalkulacyjny</param>
        /// <returns>Ilość zakładek w arkuszu kalkulacyjnym</returns>
        private static int GetNumberWorkbookPart(SpreadsheetDocument spreadsheetDocument)
        {
            WorkbookPart workbookPart = spreadsheetDocument.WorkbookPart;

            IEnumerable<Sheet> sheetcollection = spreadsheetDocument.WorkbookPart.Workbook.GetFirstChild<Sheets>().Elements<Sheet>();
            return sheetcollection.Count();
        }

        /// <summary>
        /// Metoda zwracajaca nazwę zakłądki o numerze
        /// </summary>
        /// <param name="spreadsheetDocument">Dokument excel</param>
        /// <param name="number">Numer zakładki</param>
        /// <returns></returns>
        private static string GetNumberWorkbookPart(SpreadsheetDocument spreadsheetDocument, int number)
        {
            WorkbookPart workbookPart = spreadsheetDocument.WorkbookPart;

            IEnumerable<Sheet> sheetcollection = spreadsheetDocument.WorkbookPart.Workbook.GetFirstChild<Sheets>().Elements<Sheet>();

            return sheetcollection.First(d => d.SheetId.Value == number).Name;
        }
        ...
        {
            int count = Math.Max(rowCollection.ElementAt(0).Count(), rowCollection.ElementAt(1).Count());   // tutaj inaczej bo nei zakładam nazwy w dwóch liniach
        ...
        /// <param name="rowCollection">Wiersze do przeanalizowania.</param>
        /// <returns>Zwraca mapę nazw kolumn na numer komórki w wierszu.</returns>
        private static Slownik<string, int> GetColumnsWhereOneTitleRow(SpreadsheetDocument spreadsheetDocument, IEnumerable<Row> rowCollection)
        {
        ...
        private static Cell[] GetCells(Row row, Slownik<string, int> kolumny, System.Text.StringBuilder sb, string name)
        {
            ...
            {
                if (ShouldConverted(kolumny, cells))
                {
                    if (cells.Count() < kolumny.Count())       // sprawdzenie czy nie ma pustych kolumn
                    {
                        if (sb.Length == 0)
                            sb.AppendLine("\r\n\r\nUwagi do przetwarzanych danych:");
                        sb.AppendFormat("Zakładka {0}, wiersz {1} ma nieuzupełnione kolumny. W pustych kolumnach powinien być znak \"-\"\n", name, row.RowIndex);
                        return null;
                    }
                    if (!AreRequiredCellsCompleted(kolumny, cells))
                    {
                        if (sb.Length == 0)
                            sb.AppendLine("\r\n\r\nUwagi do przetwarzanych danych:");
                        sb.AppendFormat("Zakładka {0}, wiersz {1} ma nieuzupełnione wymagane kolumny\n", name, row.RowIndex);
                        return null;
                    }
                }
                else
        ...
        /// <summary>
        /// Metoda sprawdza czy dany wiersz jest poprawną daną czy już podsumowaniem.
        /// </summary>
        /// <param name="columns"></param>
        /// <param name="cells"></param>
        /// <returns></returns>
        private static bool ShouldConverted(Slownik<string, int> columns, Cell[] cells)
        {
            var column = columns.Where(k => k.Key.Equals("Lp.") || k.Key.Equals("l.p.") || k.Key.Equals("lp.") || k.Key.Equals("lp") || k.Key.Equals("L.P.") || k.Key.Equals("LP.")
                                        || k.Key.Equals("Nr") || k.Key.Equals("nr.") || k.Key.Equals("Nr.") || k.Key.Equals("nr") || k.Key.Equals("NR") || k.Key.Equals("NR.") || k.Key.Equals("numer") || k.Key.Equals("Numer")
                                        ).FirstOrDefault();

            if (column.Key != null && cells[column.Value]?.CellValue != null)
            {
                return true;
            }
            else
        ...
        private static bool AreRequiredCellsCompleted(Slownik<string, int> columns, Cell[] cells)
        {
            ...
            if (cells.Count() < requiredColumns.Count())
                return false;

            foreach (var column in columns)
            {
                if (requiredColumns.Contains(column.Key))
                {
                    if (cells[column.Value].CellValue == null || cells[column.Value].CellValue.Text == "25") // "-"
                    {
                        return false;
                    }
                }
        ...
        class ElementHelper
        {
            ...
            /// <summary>
            /// Metoda dodająca nowy podelement z bezpośrednim podaniem jego zawartości i podaniem przestrzeni nazw.
            /// </summary>
            /// <param name="ns">Przestrzeń nazw dodawanego elementu.</param>
            /// <param name="name">Nazwa dodawanego elementu.</param>
            /// <param name="content">Zawartość dodawanego elementu. Np. tekst.</param>
            /// <returns>Zwraca nowoutworzony element.</returns>
            public ElementHelper AddElementValue(XNamespace ns, string name, object content)
            {
                var ret = new ElementHelper();
                ret.Init(ns, name, content);
                _elem.Add(ret._elem);
                return ret;
            }
            /// <summary>
